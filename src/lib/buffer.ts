...
export default class Buffer {
  ...
  store(state: number[], value: number, action: number[], probability: number) {
  ...
  createMiniBatches() {
    ...
    for (const key of this.miniBatchKeys) {
      ...
      for (
        ...
        miniBatchData.push(tf.tensor(data.slice(i, i + this.miniBatchSize)));
      }
  ...
  getMiniBatches(): tf.Tensor[][] {
    return this.miniBatchKeys.map((key) => this.container["_minibatch_" + key]);
  }
